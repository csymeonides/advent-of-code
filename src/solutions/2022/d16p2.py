...
def solve(data):
    ...
    while queue:
        my_loc, my_time, el_loc, el_time, score, open_valves = queue.pop(0)

        for other, distance in distances[my_loc].items():
            if other not in open_valves and my_time > distance + 1:
                new_time = my_time - distance - 1
                ...
                new_state = (other, new_time, el_loc, el_time, new_score, new_open_valves)
                queue.append(new_state)

        for other, distance in distances[el_loc].items():
            if other not in open_valves and el_time > distance + 1:
                new_time = el_time - distance - 1
                new_score = score + (new_time * flow_rates[other])
                new_open_valves = open_valves + [other]
                ...
                new_state = (my_loc, my_time, other, new_time, new_score, new_open_valves)
                queue.append(new_state)
