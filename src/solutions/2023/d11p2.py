...
def solve(data: List[List[str]]) -> int:
    ...
    for i in range(len(galaxies) - 1):
        ...
        for (xx, yy) in galaxies[i+1:]:
            min_x = min(x, xx)
            max_x = max(x, xx)
            distance = (yy - y) + (max_x - min_x) + (len([col for col in empty_columns if min_x < col < max_x]) * 999999) + (len([row for row in empty_rows if y < row < yy]) * 999999)
            total += distance
